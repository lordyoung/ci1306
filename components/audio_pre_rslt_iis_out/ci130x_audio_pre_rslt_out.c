...
void audio_pre_rslt_write_data_from_uart(uint32_t addr,uint32_t size)
{
    ...
    if (((uint32_t)UART_NUM_SEND_AUDIO_NUMBER == (uint32_t)UART1))
    {
        uart_dma_peripherals_num = DMAC_Peripherals_UART1_TX;
        uart_fifo_addr = UART1FIFO_BASE;
    }
    else if (((uint32_t)UART_NUM_SEND_AUDIO_NUMBER == (uint32_t)UART2))
    {
...
{
    mprintf("2-----audio_pre_rslt_out_play_card_init ....\r\n");
...
#endif
#if !USE_IIS0_AUDIO_IN_SPEAKER_OUT
    audio_pre_rslt_out_codec_init();
#endif

#if USE_HP_OUT_PRE_RSLT_AUDIO || USE_UART_SEND_PRE_RSLT_AUDIO
...
void audio_pre_rslt_write_data(int16_t* left,int16_t* right)
{
    ...
    int16_t *pcm_data_p = NULL;
    int num = 0, ret = 0;
#if USE_IIS0_OUT_PRE_RSLT_AUDIO || USE_HP_OUT_PRE_RSLT_AUDIO
#if USE_UART_SEND_PRE_RSLT_AUDIO
    pcm_data_p = (int16_t *)tmp_voice_addr;
    ...
    else if (cnt >= 30)
    {
        ...
        if(speex_init_flag)
        {
         ...
         //   mprintf("xSpeexRecordStreamBuffer send %d\r\n",ret);
           // speex_mutex_give();
          
            //audio_pre_rslt_write_data_from_uart((uint32_t)left, 256*2);

         //  int encode_len = cias_speex_compressed_data(ci_speex_hander, src_addr, dst_addr, 320);     //压缩音频数据    //       // mprintf("block_size = %d\r\n", block_size);
         //  audio_pre_rslt_write_data_from_uart((uint32_t)dst_addr, encode_len);
          // mprintf("encode_len = %d\r\n", encode_len);

    //    // audio_pre_rslt_write_data_from_uart((uint32_t)left, block_size/2);
        }
